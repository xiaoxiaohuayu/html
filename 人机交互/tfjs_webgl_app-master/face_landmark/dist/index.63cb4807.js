...
pmeter.reset_lap = function(dpy_id) {
    if (dpy_id >= PMETER_DPY_NUM) return;
    pmeter.lap_cnt[dpy_id] = 0;
};
pmeter.set_lap = function(dpy_id) {
    if (dpy_id >= PMETER_DPY_NUM) return;
    if (pmeter.lap_cnt[dpy_id] >= PMETER_MAX_LAP_NUM) return;
...
pmeter.resize = function(gl, win_w, win_h, data_num) {
    ...
    pmeter.wndH = win_h;
    pmeter.data_num = data_num;
    for(let dpy = 0; dpy < PMETER_DPY_NUM; dpy++)for(let i = 0; i < PMETER_MAX_LAP_NUM; i++){
        pmeter.vertex[dpy][i] = new Array(data_num * 2);
        for(let j = 0; j < data_num; j++){
            pmeter.vertex[dpy][i][2 * j] = 0;
            pmeter.vertex[dpy][i][2 * j + 1] = j;
...
pmeter.set_pmeter_val = function(dpy_id, id, val) {
    if (id >= PMETER_MAX_LAP_NUM) return;
    if (dpy_id >= PMETER_DPY_NUM) return;
    if (val > 100) val = 100;
...
pmeter.draw_pmeter_ex = function(gl, dpy_id, x, y, scale) {
    ...
    let sumval = 0;
    for(let i = 0; i < num_lap; i++)sumval += laptime[i];
    ...
    if (pmeter.cursor[dpy_id] >= pmeter.data_num) pmeter.cursor[dpy_id] = 0;
    gl.useProgram(pmeter.sobj.program);
    gl.uniform4f(pmeter.locPrjMul, scale * 2 / pmeter.wndW, -2 / pmeter.wndH, 0, 0);
    gl.uniform4f(pmeter.locPrjAdd, -1, 1, 1, 1);
    ...
    gl.uniform4f(pmeter.locCol, 0.5, 0.5, 0.5, 1);
    for(let i1 = 1; i1 <= 10; i1++){
        gl.uniform4f(pmeter.locTrans, x + i1 * 10, y, 0, 0);
        gl.drawArrays(gl.LINES, 0, 2);
    }
    /* GRAPH */ gl.uniform4f(pmeter.locTrans, x, y, 0, 0);
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(pmeter.vertex[dpy_id][0]), gl.STATIC_DRAW);
    gl.vertexAttribPointer(pmeter.locVtx, 2, gl.FLOAT, false, 0, 0);
    gl.uniform4f(pmeter.locCol, 1, 0, 0, 1);
...
*/ /* CURSOR */ var vert2 = [
    ...
    gl.vertexAttribPointer(pmeter.locVtx, 2, gl.FLOAT, false, 0, 0);
    gl.uniform4f(pmeter.locCol, 0, 1, 0, 1);
    gl.uniform4f(pmeter.locTrans, x, y + pmeter.cursor[dpy_id], 0, 0);
    gl.lineWidth(3);
    gl.drawArrays(gl.LINES, 0, 2);
    gl.lineWidth(1);
};
pmeter.draw_pmeter = function(gl, x, y) {
    pmeter.draw_pmeter_ex(gl, 0, x, y, 1);
};

//# sourceMappingURL=index.63cb4807.js.map
