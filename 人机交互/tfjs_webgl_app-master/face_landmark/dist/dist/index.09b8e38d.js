...
***********************************************************/ function matrix_rotate(m, angle, x, y, z) {
    ...
    /* for fast rotation around X-Axis/Y-Axis,and Z-Axis */ if (x == 0 && y == 0 && z != 0) {
        if (z < 0) /* If the Axis of the Rotation is minus, Rotate Backwords */ sinA = -sinA;
        /* Z Axis Rotateion */ matrix_turn_z(m, cosA, sinA);
        return;
    } else if (x == 0 && y != 0 && z == 0) {
        if (y < 0) /* If the Axis of the Rotation is minus, Rotate Backwords */ sinA = -sinA;
        /* Y Axis Rotation */ matrix_turn_y(m, cosA, sinA);
        return;
    } else if (x != 0 && y == 0 && z == 0) {
        if (x < 0) /* If the Axis of the Rotation is minus, Rotate Backwords */ sinA = -sinA;
        /* X Axis Rotation */ matrix_turn_x(m, cosA, sinA);
        return;
    }
    {
        let r00, r01, r02;
        let r10, r11, r12;
        let r20, r21, r22;
        /* normalization of 3D-vector */ v[0] = x;
        v[1] = y;
        v[2] = z;
        vec3_normalize(v);
        x = v[0];
        y = v[1];
        z = v[2];
        /* making rotation matrix */ cosA2 = 1 - cosA;
        xsinA = x * sinA;
        ysinA = y * sinA;
        zsinA = z * sinA;
        xcosA2 = x * cosA2;
...
function matrix_proj_frustum(mat, left, right, bottom, top, znear, zfar) {
    ...
    mat[0] = 2 * znear / (right - left);
    mat[5] = 2 * znear / (top - bottom);
...
function matrix_proj_perspective(mat, fovy, aspect, znear, zfar) {
    let a, b, c, d, f;
    mat[0] = mat[1] = mat[2] = mat[3] = mat[4] = mat[5] = mat[6] = mat[7] = mat[8] = mat[9] = mat[10] = mat[11] = mat[12] = mat[13] = mat[14] = mat[15] = 0;
    f = Math.cos(6.28 * (fovy / 2) / 360) / Math.sin(6.28 * (fovy / 2) / 360);
    a = f / aspect; /* a = f/aspect                */ 
    b = f; /* b = f                       */ 
    c = (zfar + znear) / (znear - zfar); /* c = (far+near)/(near-far)   */ 
    d = 2 * zfar * znear / (znear - zfar); /* d = (2*far*near)/(near-far) */ 
    mat[0] = a;
    mat[5] = b;
    mat[10] = c;
    mat[11] = -1;
...
function matrix_invert(m) {
    ...
    if (m03 == 0 && m07 == 0 && m11 == 0 && m15 == 1) {
        ...
        /* M^-1[ij] = invdet * M[ji] */ m[0] = W00 * invdet;
        m[4] = W01 * invdet;
        m[8] = W02 * invdet;
        m[12] = W03 * invdet;
        m[1] = W04 * invdet;
...
/* lpR = lpP * lpQ */ function quaternion_mult(lpR, lpP, lpQ) {
    let pw, px, py, pz;
    let qw, qx, qy, qz;
    pw = lpP[0];
    px = lpP[1];
...
//# sourceMappingURL=index.09b8e38d.js.map
