...
GLUtil.create_2d_texture = function(gl, imgbuf, width, height) {
    ...
    gl.bindTexture(gl.TEXTURE_2D, texid);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, width, height, 0, gl.RGBA, gl.UNSIGNED_BYTE, imgbuf);
    return texid;
};
/* ---------------------------------------------------------------- *
 ...
 * ---------------------------------------------------------------- */ GLUtil.create_video_texture = function(gl, url) {
    ...
    video.play();
    function checkReady() {
        if (playing && timeupdate) video_tex.ready = true;
    }
 ...
 * ---------------------------------------------------------------- */ GLUtil.create_camera_texture = function(gl) {
    ...
    video.autoplay = true;
    video.loop = true;
    navigator.mediaDevices = navigator.mediaDevices || navigator.mozGetUserMedia;
    ...
    promise.then(on_camera_ready).catch(on_camera_failed);
...
//# sourceMappingURL=index.2ec8baa1.js.map
