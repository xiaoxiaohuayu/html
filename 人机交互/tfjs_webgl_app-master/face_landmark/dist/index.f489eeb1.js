 ...
 */ function calc_size_to_fit(gl, src_w, src_h, win_w, win_h) {
    ...
    let region = {
        width: win_w,
        /* full rect width  with margin */ height: win_h,
        /* full rect height with margin */ tex_x: offset_x,
        /* start position of valid texture */ tex_y: offset_y,
        /* start position of valid texture */ tex_w: scaled_w,
        /* width  of valid texture */ tex_h: scaled_h,
        /* height of valid texture */ scale: scale
    };
    return region;
}
function render_2d_scene(gl, texid, face_predictions, tex_w, tex_h, masktex, mask_predictions) {
    ...
    let mask_color = [
        ...
        1,
        1,
        s_gui_prop.mask_alpha
    ];
    if (s_is_dragover) mask_color = [
        0.8,
        0.8,
        0.8,
        1
    ...
    for(let i = 0; i < face_predictions.length; i++){
        ...
        /* render the deformed mask image onto the camera image */ if (mask_predictions.length > 0) {
...
function init_gui() {
    ...
    gui.add(s_gui_prop, 'mask_alpha', 0, 1);
 ...
 * ---------------------------------------------------------------- */ function startWebGL() {
    ...
    let facemesh_model;
    {
        ...
        let promise = faceLandmarksDetection.load(faceLandmarksDetection.SupportedPackages.mediapipeFacemesh);
        promise.then(on_facemesh_model_load).catch(on_facemesh_model_load_failed);
    }
    current_phase = 1;
    ...
    async function render(now) {
        ...
        if (facemesh_ready) {
            if (mask_init_done == false) {
                for(let i = 0; i < 5; i++)/* repeat 5 times to flush pipeline ? */ mask_predictions = await facemesh_model.estimateFaces({
                    input: masktex.image
                });
                mask_init_done = true;
            ...
            if (mask_update_req) /* if readfile has done, update face mask */ {
                if (masktex_next.image.width > 0) {
                    for(let i = 0; i < 5; i++)/* repeat 5 times to flush pipeline ? */ mask_predictions = await facemesh_model.estimateFaces({
                        input: masktex_next.image
                    });
        ...
        if (facemesh_ready) {
            ...
            for(let i = 0; i < num_repeat; i++)if (GLUtil.is_camera_ready(camtex)) face_predictions = await facemesh_model.estimateFaces({
                input: camtex.video
            });
            else face_predictions = await facemesh_model.estimateFaces({
                input: imgtex.image
            });
