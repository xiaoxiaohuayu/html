 ...
 * ---------------------------------------------------------------- */ function touch_client_coord(canvas, event, id) {
    let rect = canvas.getBoundingClientRect();
    return {
        x: event.changedTouches[id].pageX - rect.left,
...
function get_touch_idx(key_id) {
    for(let i = 0; i < s_ev.ongoing_touch.length; i++){
        if (s_ev.ongoing_touch[i].id == key_id) return i;
    }
    return -1;
}
...
function on_touch_end(event) {
    ...
    for(let i = 0; i < event.changedTouches.length; i++){
        ...
        if (idx >= 0) s_ev.ongoing_touch.splice(idx, 1); /* remove it */ 
    }
}
function on_touch_move(event) {
    ...
    for(let i = 0; i < event.changedTouches.length; i++){
        ...
        if (idx >= 0) s_ev.ongoing_touch.splice(idx, 1, mouse_pos); /* swap in the new touch record */ 
    //if (i == 0)
...
function init_touch_event(canvas) {
...
function get_touch_event_matrix() {
    quaternion_to_matrix(s_ev.mdl_mtx, s_ev.mdl_qtn);
    s_ev.mdl_mtx[14] += s_ev.wheel_pos;
    return s_ev.mdl_mtx;
}

//# sourceMappingURL=index.d8027231.js.map
