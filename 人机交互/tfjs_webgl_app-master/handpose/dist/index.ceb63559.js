...
shapes.gen_shape_buffers = function(gl, nDivU, nDivV, pshape) {
    ...
    for(let i = 0; i < nDivU - 1; i++)for(let j = 0; j < nDivV - 1; j++){
...
shapes.generate_shape = function(gl, func, sparam) {
    ...
    for(let i = 0; i < nSampleU; i++)for(let j = 0; j < nSampleV; j++){
    ...
    for(let i1 = 0; i1 < nSampleU; i1++)for(let j1 = 0; j1 < nSampleV; j1++){
        pUV[(j1 * nSampleU + i1) * 2 + 0] = i1 / (nSampleU - 1);
        pUV[(j1 * nSampleU + i1) * 2 + 1] = j1 / (nSampleV - 1);
    }
    for(let i2 = 0; i2 < nSampleU; i2++)for(let j2 = 0; j2 < nSampleV; j2++){
        pColor[(j2 * nSampleU + i2) * 3 + 0] = i2 / (nSampleU - 1);
        pColor[(j2 * nSampleU + i2) * 3 + 1] = 1 - i2 / (nSampleU - 1);
        pColor[(j2 * nSampleU + i2) * 3 + 2] = j2 / (nSampleV - 1);
    }
    for(let i3 = 0; i3 < nSampleU - 1; i3++)for(let j3 = 0; j3 < nSampleV - 1; j3++){
        var ptA = [], ptB = [], ptC = [], AB = [], AC = [], normal = [];
        ptA[0] = pVertex[(j3 * nSampleU + i3) * 3 + 0];
        ptA[1] = pVertex[(j3 * nSampleU + i3) * 3 + 1];
        ptA[2] = pVertex[(j3 * nSampleU + i3) * 3 + 2];
        ptB[0] = pVertex[(j3 * nSampleU + i3 + 1) * 3 + 0];
        ptB[1] = pVertex[(j3 * nSampleU + i3 + 1) * 3 + 1];
        ptB[2] = pVertex[(j3 * nSampleU + i3 + 1) * 3 + 2];
        ptC[0] = pVertex[((j3 + 1) * nSampleU + i3) * 3 + 0];
        ptC[1] = pVertex[((j3 + 1) * nSampleU + i3) * 3 + 1];
        ptC[2] = pVertex[((j3 + 1) * nSampleU + i3) * 3 + 2];
        AB[0] = ptB[0] - ptA[0];
        AB[1] = ptB[1] - ptA[1];
        AB[2] = ptB[2] - ptA[2];
        AC[0] = ptC[0] - ptA[0];
        ...
        pNormal[(j3 * nSampleU + i3) * 3 + 0] = -normal[0];
        pNormal[(j3 * nSampleU + i3) * 3 + 1] = -normal[1];
        pNormal[(j3 * nSampleU + i3) * 3 + 2] = -normal[2];
        shapes.normalize(AB);
        pTangent[(j3 * nSampleU + i3) * 3 + 0] = -AB[0];
        pTangent[(j3 * nSampleU + i3) * 3 + 1] = -AB[1];
        pTangent[(j3 * nSampleU + i3) * 3 + 2] = -AB[2];
    }
    for(let i4 = 0; i4 < nSampleU - 1; i4++){
        pNormal[((nSampleV - 1) * nSampleU + i4) * 3 + 0] = pNormal[i4 * 3 + 0];
        pNormal[((nSampleV - 1) * nSampleU + i4) * 3 + 1] = pNormal[i4 * 3 + 1];
        pNormal[((nSampleV - 1) * nSampleU + i4) * 3 + 2] = pNormal[i4 * 3 + 2];
        pTangent[((nSampleV - 1) * nSampleU + i4) * 3 + 0] = pTangent[i4 * 3 + 0];
        pTangent[((nSampleV - 1) * nSampleU + i4) * 3 + 1] = pTangent[i4 * 3 + 1];
        pTangent[((nSampleV - 1) * nSampleU + i4) * 3 + 2] = pTangent[i4 * 3 + 2];
    }
    for(let j4 = 0; j4 < nSampleV - 1; j4++){
        pNormal[(j4 * nSampleU + nSampleU - 1) * 3 + 0] = pNormal[j4 * nSampleU * 3 + 0];
        pNormal[(j4 * nSampleU + nSampleU - 1) * 3 + 1] = pNormal[j4 * nSampleU * 3 + 1];
        pNormal[(j4 * nSampleU + nSampleU - 1) * 3 + 2] = pNormal[j4 * nSampleU * 3 + 2];
        pTangent[(j4 * nSampleU + nSampleU - 1) * 3 + 0] = pTangent[j4 * nSampleU * 3 + 0];
        pTangent[(j4 * nSampleU + nSampleU - 1) * 3 + 1] = pTangent[j4 * nSampleU * 3 + 1];
        pTangent[(j4 * nSampleU + nSampleU - 1) * 3 + 2] = pTangent[j4 * nSampleU * 3 + 2];
    }
...
shapes.func_KleinBottle = function(u, v) {
    let botx = 4 * Math.cos(u) * (1 + Math.sin(u));
    let boty = 12 * Math.sin(u);
    let rad = 3 * (1 - Math.cos(u) / 2);
    let x, y, z;
    if (u > 1.7 * Math.PI) {
...
shapes.func_BoySurface = function(u, v) {
    ...
    let y = (b * b - c * c + c * a * (c * c - a * a) + a * b * (b * b - a * a)) * Math.sqrt(3) / 2;
 ...
 * -------------------------------------------------------------------------- */ shapes.shape_create = function(gl, type, nDivU, nDivV) {
    ...
    switch(type){
        ...
        case shapes.SHAPE_MOEBIUS:
            ...
            sparam.min_u = -Math.PI / 6;
            sparam.max_u = Math.PI / 6;
            sparam.min_v = 0;
            sparam.max_v = 2 * Math.PI;
            break;
...
//# sourceMappingURL=index.ceb63559.js.map
