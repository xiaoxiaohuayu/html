...
let s_srctex_region = {
    width: 0,
    height: 0,
    /* full rect with margin */ tex_x: 0,
    tex_y: 0,
    tex_w: 0,
    tex_h: 0 /* valid texture area */ 
};
function init_stats() {
    var stats = new Stats();
...
function compute_3d_skelton_pos(dst_pose, src_pose) {
    for(let i = 0; i < src_pose.length; i++){
        ...
        z = (z - 0) * s_gui_prop.pose_scale_z * 10;
        y = -y;
        z = -z;
        dst_pose.push([
            x,
...
function render_3d_bone(gl, mtxGlobal, pose, idx0, idx1, color, rad, is_shadow) {
    const pos0 = pose[idx0];
    const pos1 = pose[idx1];
    draw_bone(gl, mtxGlobal, pos0, pos1, rad, color, is_shadow);
}
function render_palm_tri(gl, mtxGlobal, pose, idx0, idx1, idx2, color) {
    const pos0 = pose[idx0];
    const pos1 = pose[idx1];
...
function render_skelton_3d(gl, landmarks) {
    ...
    for(let is_shadow = 1; is_shadow >= 0; is_shadow--){
        ...
        if (is_shadow) {
            ...
            matrix_translate(mtxGlobal, 0, shadow_y, 0);
            matrix_mult(mtxGlobal, mtxGlobal, mtxShadow);
            colj = col_gray;
            coln = col_gray;
            colp = col_gray;
        }
        /* joint point */ for(let i = 0; i < pose.length; i++){
            if (!is_shadow) {
        ...
        /* palm node */ const rad = s_gui_prop.bone_radius;
        ...
        /* finger node */ for(let i1 = 0; i1 < 5; i1++){
            let idx0 = 4 * i1 + 1;
            let idx1 = idx0 + 1;
...
function render_2d_bone(gl, landmarks, idx0, idx1, scale, ox, oy) {
    ...
    r2d.draw_2d_line(gl, p0[0] * scale + ox, p0[1] * scale + oy, p1[0] * scale + ox, p1[1] * scale + oy, color, 1);
}
function render_2d_scene(gl, texid, hand_predictions, tex_w, tex_h) {
    let color = [
        0,
        1,
        1,
    ...
    r2d.draw_2d_rect(gl, tx, ty, tw, th, [
        0,
        1,
        1,
        1
...
function flip_h_predictions(hand_predictions, tex_w) {
    for(let i = 0; i < hand_predictions.length; i++){
...
function render_progress_bar(gl, current_phase, hand_predictions, win_w, win_h) {
    ...
    r2d.draw_2d_fillrect(gl, x, y, w, h, [
        0,
        0.4,
        0.4,
        0.2
    ]);
    r2d.draw_2d_fillrect(gl, x, y, wp, h, [
        0,
        0.4,
        0.4,
        0.5
    ]);
    r2d.draw_2d_rect(gl, x, y, w, h, [
        0,
        1,
        1,
        0.8
    ...
    if (current_phase < 2) {
        ...
        dbgstr.draw_dbgstr_ex(gl, str, x, y, 1, [
            0,
            1,
            1,
            1
        ...
        dbgstr.draw_dbgstr_ex(gl, str, x, y + 22, 1, [
            0,
            1,
            1,
            1
...
function init_gui() {
    ...
    gui.add(s_gui_prop, 'srcimg_scale', 0, 5);
 ...
 * ---------------------------------------------------------------- */ function startWebGL() {
    ...
    let handpose_model;
    {
        ...
        promise.then(on_model_load).catch(on_model_load_failed);
    }
    current_phase = 1;
    /* stop loading spinner */ const spinner = document.getElementById('loading');
    ...
    async function render(now) {
        ...
        if (handpose_ready) {
            ...
            let flip_h = false;
            if (GLUtil.is_camera_ready(camtex)) hand_predictions = await handpose_model.estimateHands(camtex.video, flip_h);
            else hand_predictions = await handpose_model.estimateHands(imgtex.image, flip_h);
            time_invoke0 = performance.now() - time_invoke0_start;
            if (s_gui_prop.flip_horizontal) flip_h_predictions(hand_predictions, src_w);
        }
        /* --------------------------------------- *
         *  render scene
        ...
        dbgstr.draw_dbgstr(gl, str, win_w - 220, win_h - 44);
        str = "srcdim(" + src_w + ", " + src_h + ")";
        dbgstr.draw_dbgstr(gl, str, win_w - 220, win_h - 22);
        //stats.end();
...
//# sourceMappingURL=index.f489eeb1.js.map
